import { Button, Checkbox, FormControl, Input, InputLabel, ListItemText, makeStyles, MenuItem, Select } from '@material-ui/core';
...
const MultiSelect = props => {
  ...
  const handleChange = (event, type, typeId) => {
  ...
  const onSearch = () => {
    ...
    dispatch(Actions.Filter.update({ key: "products", value: { ...filter, categories } }));
  };
  ...
  return (
    <div className={classes.root}>
      <FormControl disabled className={classes.formControl}>
        <InputLabel id="mutiple-checkbox-label">Platform</InputLabel>
        <Select
          labelId="mutiple-checkbox-label"
          id="mutiple-checkbox"
          multiple
          value={platformName}
          onChange={(e, v) => handleChange(e, "platform")}
          input={<Input />}
          MenuProps={MenuProps}
        >
          {platforms.map((p, index) => (
            <MenuItem key={index} value={p.name}>
              <Checkbox checked={platformName.indexOf(p) > -1} />
      ...
      <FormControl className={classes.formControl}>
        ...
        >
          ...
          {categories.map((c, index) => (
            ...
            // <MenuItem key={index} value={c.name} onClick={handleClick}>
            //   {c.name}
            // </MenuItem>
          ))}
          {/* {productType && categoryLists.map((c, index) => (
            <MenuItem key={index} value={c.child?.id} onClick={handleClick}>
              {c.child?.name}
            </MenuItem>
          ))} */}
        ...
        {/* {!!categoryLists.length && (
          ...
          >
            {categoryLists.map((c, index) => (
              <MenuItem key={index} value={c.id} onClick={(e) => handleChange(e, "type", c.id)}>
                {c.child?.name}
              </MenuItem>
            ))}
          </Menu>
