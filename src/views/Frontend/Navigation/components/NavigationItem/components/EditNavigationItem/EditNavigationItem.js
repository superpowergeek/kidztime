...
import { DeleteDialog, EmptyState, ErrorMessage, InputGroup, LoadButton, PopUpDialog } from "components";
...
import { formStructure, messages } from "./editNavigationItemConfig";
...
const EditNavigationItem = (props) => {
  const { open, handleClose, model: parent } = props;
  const api = useApi();
  const classes = useStyles();
  const [runCreate, load, error, setError] = useAsyncTask("navigation-item-edit");
  const [remove, setRemove] = useState(false);
  const initialisedFields = {
    ordering: parent?.banner?.extra0,
    title: parent?.banner?.value
  }
  ...
  const [fields, values, errors, setValues, setErrors, setDirtys] = useFormHandler(formStructure, initialisedFields); // eslint-disable-line
  ...
  const del = () => {
    if (parent?.banner?.id) {
      return api.path("objectmeta/banner/delete", { objectmeta_id: parent?.banner?.id }).del();
    }
  };
  ...
  const handleCreate = (ev) => {
    ...
    runCreate(async () => {
      ...
      await api.path("objectmeta/banner/update", { objectmeta_id: parent?.banner?.id }).post({ body });
      handleClose();
      enqueueSnackbar("Navigation Item Successfully Edited!", { variant: "success" });
    });
  }
